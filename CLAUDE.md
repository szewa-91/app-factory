...
- **`scripts/`**: 
  ...
  - `scheduler.sh`: Production cron entrypoint wrapper for the compiled TypeScript scheduler at `scripts/scheduler/dist/index.js`.
  - `scheduler/src/`: TypeScript scheduler source modules (lock handling, cycle orchestration, auditing, failure context).
  - Rebuild command after scheduler source changes: `cd scripts/scheduler && npm run build`.
